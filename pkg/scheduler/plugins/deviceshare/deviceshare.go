...
const (
	...
	//节点内GPU选择策略
	GPUSelectPolicyArgument = "deviceshare.GPUSelectPolicy"
	DQNGRPCEndpointArgument = "deviceshare.DQNGRPCEndpoint"
...
func enablePredicate(dsp *deviceSharePlugin) {
	...
	vgpu.GPUSelectPolicy = vgpu.OriginalPolicy
	args.GetString(&vgpu.GPUSelectPolicy, GPUSelectPolicyArgument)
	klog.Infof("deviceshare GPUSelectPolicy=%s", vgpu.GPUSelectPolicy)

	args.GetString(&vgpu.DQNGRPCEndpoint, DQNGRPCEndpointArgument)
	klog.Infof("deviceshare DQNGRPCEndpoint=%s", vgpu.DQNGRPCEndpoint)
